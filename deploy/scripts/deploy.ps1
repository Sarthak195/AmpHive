...
if (-not $NoTls) {
    ...
    $caddy_lines += "`t# Browser security headers (SECURITY.md 3)."
    $caddy_lines += "`theader {"
    $caddy_lines += "`t`tStrict-Transport-Security `"max-age=31536000; includeSubDomains`""
    $caddy_lines += "`t`tX-Content-Type-Options `"nosniff`""
    $caddy_lines += "`t`tX-Frame-Options `"DENY`""
    $caddy_lines += "`t`t# style/font third-party origins dropped 2026-07-21: Leaflet CSS +"
    $caddy_lines += "`t`t# Inter are bundled/self-hosted now (audit follow-up). MUST be"
    $caddy_lines += "`t`t# browser-smoke-tested after deploy (Playwright), not just curl'd."
    $caddy_lines += "`t`tContent-Security-Policy `"default-src 'self'; script-src 'self' https://*.razorpay.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https: wss:; frame-src https://*.razorpay.com; base-uri 'self'; form-action 'self'`""
    $caddy_lines += "`t}"
