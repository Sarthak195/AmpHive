...
if (-not $NoTls) {
    ...
    $caddy_lines += "`t`tContent-Security-Policy `"default-src 'self'; script-src 'self' https://*.razorpay.com; style-src 'self' 'unsafe-inline' https://unpkg.com https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https: wss:; frame-src https://*.razorpay.com; base-uri 'self'; form-action 'self'`""
    $caddy_lines += "`t}"
