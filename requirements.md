...
- **Centralized PaaS Billing:** Drivers enter the Plug ID (printed on the charging bay) into the app to authenticate, start a session, and pay via a virtual wallet. Private charger groups require an access code to join. The central platform coordinates plug commands and logs timeseries telemetry.
...
- **Public Chargers** – Plugs that can be discovered and used by any registered user. Their location is shown on a map and they are listed in the marketplace. Users enter the Plug ID directly to start a session.

- **Private Groups** – Restricted chargers gated by an **access code**:
  - CPOs create named groups (e.g., "Sunrise Apartments", "TechPark Office", "Personal Home").
  - Each group has a unique, shareable **access code** generated by the CPO.
  - Users enter the access code in the app to **join** the group and unlock its chargers.
  - Once joined, the group's plugs appear in the user's dashboard.
  - Group types can include: *Personal*, *Society*, or *Office* — the access code is the universal gating mechanism.

> **Note:** No QR code scanning is required. Users simply enter the printed Plug ID into the mobile app or web portal to start a charging session. For private chargers, the user must first join the group via access code.
...
- **Plug lookup** within the app (search by Plug ID, map view, autocomplete).
...
- **Razorpay payment integration** (UPI, cards, wallets, net banking) and automated invoicing.
...
- **Driver (End-User):** Can register an account, top up a prepaid virtual wallet (via Razorpay), enter Plug IDs to start/stop charging, join private groups via access codes, and view active charge speeds and cost histories.

### 5.2. Core Workflows
#### Session Start Workflow
1. Driver enters the Plug ID (printed on the charging bay label) into the AmpHive web application.
2. The system verifies the user has access to the plug (public, or member of the plug's private group). Then checks the user's prepaid wallet balance. A minimum balance is required to start (e.g., ₹50).
