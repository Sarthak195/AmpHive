...
REM ---------------------------------------------------------------------
echo Stopping the relay stack on amphive-relay...
echo NOTE: this disconnects every fielded gateway from the MQTT broker.
gcloud compute ssh amphive-relay --zone=us-west1-a --command="cd ~/amphive-relay && sudo docker compose -f docker-compose.relay.yml down"
pause
