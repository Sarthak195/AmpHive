...
| OTA hardening: signed images + https-only | 🟡 | **Code complete 2026-07-10** (fw ≥ 1.4.0): ECDSA signed-app verification on update (`SECURE_SIGNED_ON_UPDATE_NO_SECURE_BOOT`, key gitignored), plain http refused by firmware + backend (`CpoGatewayOtaRequest` `^https://`), images published to the public HTTPS bucket `gs://amphive-fw`. Signed `1.4.0-direct` built, signature-verified, and uploaded; **on-device push + backend deploy pending** (`deploy/docs/ota_image_publishing.md`). |
