...
| Frontend tests (Vitest + RTL) | ✅ | 75 tests: AuthContext, ProtectedRoute/CpoProtectedRoute (incl. the `state.from` redirect target), Login (returns to the preserved origin, incl. `?plug=`), TopUp payment handler (no client amount on `/verify`), multi-session SessionContext, History, Home (QR/deep-link prefill + auth gate + unknown-id notice, private/public sections + collapsed-map behavior, availability/group filters, legend + section counts), MapComponent (marker color per state, fit-bounds vs country-wide fallback), CpoPlugs QR modal. `npm test` runs in CI. |
