...
| Web HTTPS (Caddy front door) | 🟡 | **Code complete 2026-07-11, rollout pending.** `deploy.ps1` now ships `docker-compose.tls.yml` by default (`-NoTls` = plain-HTTP rollback): Caddy terminates HTTPS on 80/443 with auto Let's Encrypt for `CADDY_DOMAIN` (Caddyfile generated on the VM from `.env`), HTTP + bare-IP requests redirect to the canonical https origin, and the frontend container stops publishing a host port. The tls compose was brought to prod parity first (8883 TLS listener, passwd/ACL/cert mounts, authed healthcheck — it predated direct-MQTT and would have broken the broker). Firewall rule `allow-amphive-https` (tcp:443) created + DNS verified (`amphive.duckdns.org` → `8.231.81.12`, updater cron on the VM) 2026-07-11. **The next `deploy.ps1` run makes prod HTTPS** — runbook: `deploy/docs/web_tls_rollout.md`. Backend `:8000` stays public plain-HTTP for now; close after verification (SECURITY.md §3). |
