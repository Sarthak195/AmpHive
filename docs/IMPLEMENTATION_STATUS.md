...
| Web HTTPS (Caddy front door) | 🟡 | **Deployed 2026-07-11; cert issuance pending a DuckDNS outage.** `deploy.ps1` ships `docker-compose.tls.yml` by default (`-NoTls` = plain-HTTP rollback): Caddy is the only public web entrypoint on 80/443, auto Let's Encrypt for `CADDY_DOMAIN` (Caddyfile generated on the VM from `.env`), frontend container publishes no host port; bare-IP/unknown-Host requests are *served* (not redirected) so a DNS-provider outage can't take the site down. The tls compose was brought to prod parity first (8883/passwd/ACL/cert mounts — it predated direct-MQTT and would have broken the broker). **Verified in prod through the front door**: SPA, `/api`, Socket.io handshake, CPO login + gateway list (broker untouched — both gateways stayed online). The Let's Encrypt cert itself is blocked by DuckDNS's authoritative nameservers SERVFAILing during the rollout window; Caddy auto-retries until it lands (`deploy/docs/web_tls_rollout.md` incident log). Follow-ups: close public `:8000`, HSTS, and a **real domain** (DuckDNS just demonstrated it's a SPOF). |
