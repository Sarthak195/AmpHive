...
| Frontend tests (Vitest + RTL) | ✅ | 60 tests: AuthContext, ProtectedRoute/CpoProtectedRoute (incl. the `state.from` redirect target), Login (returns to the preserved origin, incl. `?plug=`), TopUp payment handler (no client amount on `/verify`), multi-session SessionContext, History, Home (QR/deep-link prefill + auth gate + unknown-id notice, availability/group filters, legend counts), MapComponent (marker color per state), CpoPlugs QR modal. `npm test` runs in CI. |
