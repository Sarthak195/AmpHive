...
- **Broker:** Eclipse Mosquitto 2.0, plain MQTT (no TLS). Confidentiality comes
  from the overlay tunnel, not from MQTT. **Auth is staged** (2026-07-07): a
  passwd file exists (backend + gateway accounts, generated by `deploy.ps1`)
  and both clients present credentials, but `allow_anonymous` stays `true`
  until the stage-2 flip — see [SECURITY.md §3](SECURITY.md).
