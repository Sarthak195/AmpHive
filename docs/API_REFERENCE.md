...
|--------|------|------|------|----------|
| POST | `/api/auth/register` | none | `{email, password, full_name}` | **(2026-08-11)** Creates an **UNVERIFIED** `driver` (`coin_balance=0`, `email_verified=false`) and emails a single-use verification link (`FRONTEND_ORIGIN/verify-email?token=...`, `EMAIL_VERIFICATION_TTL_MIN` expiry) — it does **NOT** log in / return a JWT. → `{status:"verification_sent", email}`. 400 on duplicate email. |
| POST | `/api/auth/verify-email` | none | `{token}` | **(2026-08-11)** Consumes the verification token: flips `users.email_verified` true, stamps the token used, bumps `users.token_version` (belt-and-braces), and logs the user in → `{token, user}` (`AuthResponse`). Uniform **400 "Invalid or expired verification link."** for unknown/expired/already-used tokens. Rate-limited (`VERIFY_EMAIL_RATE_LIMIT`). |
| POST | `/api/auth/resend-verification` | none | `{email}` | **(2026-08-11)** Re-issues a verification link for an existing **unverified** account (voids prior unused tokens). **Always the same generic 200 `{status:"ok"}`** whether or not the account exists / is already verified — no account enumeration. Rate-limited per IP (`RESEND_VERIFICATION_RATE_LIMIT`) and per submitted email (`RESEND_VERIFICATION_EMAIL_RATE_LIMIT`, enumeration-safe). 429 when a cap trips. |
| POST | `/api/auth/login` | none | `{email, password}` | `{token, user}` — 401 on bad credentials; **403 `account_disabled`** for an admin-disabled account (checked AFTER the password, so it's only shown to the real owner — no disabled-account oracle; redesign/ui-v3, migration `0025_user_disable`); **403 "Please verify your email address. …"** for an unverified account (2026-08-11, same after-password placement — no verified-status oracle; grandfathered pre-feature users are verified). |
...
| GET | `/api/auth/me/export` | JWT | — | **(2026-08-18)** Self-service **data export** (DPDP §11-shaped). Returns one JSON document — served as a `Content-Disposition: attachment` — with everything the platform holds *about the caller*: account, charging sessions (with charger + operator names), wallet ledger, GST invoice metadata, disputes, charger reports, reservations, group memberships, favourites, watches, notifications, and push **devices**. Strictly self-scoped: every collection filters on the caller's own `user_id`/`driver_user_id` (`services/data_export.py`). **Never** includes credentials — no password hash, token digest, or push subscription key; a push device is reported as its service HOST only (the full endpoint is a live push credential). Each collection is capped at `MAX_ROWS_PER_COLLECTION` (5000) with the collection named in `truncated_collections` rather than silently cut; per-second telemetry is deliberately excluded and the document says so. Rate-limited per account (`DATA_EXPORT_ACCOUNT_RATE_LIMIT`, default 5/3600) — it is the most expensive self-service read in the API. |
| DELETE | `/api/auth/me` | JWT | `{confirm:"DELETE MY ACCOUNT", password?}` | **(2026-08-18)** Self-service **account closure** (DPDP §12-shaped). `confirm` must be the exact phrase (400 otherwise) — a typed phrase, not a flag, because closure is irreversible and forfeits credit. `password` re-authenticates and is REQUIRED unless `auth_provider == "google"` (a Google-only account's stored hash is a deliberately unusable random value, so no password could verify — the bearer token is the proof of identity there, as on every other route); 403 on a wrong/missing password. **Anonymises rather than deletes** (`services/account_closure.py`): every `user_id` FK is `ON DELETE CASCADE`, so a real row delete would take the account's `charging_sessions`, `ledger_transactions` and GST `invoices` with it — records the operator must keep and which feed the CPO's earnings and payout watermark. So: **purged** (push subscriptions, notifications, favourites, watches, group memberships, capacity requests, outstanding reset/verification tokens), **cancelled** (BOOKED reservations, WAITING queued charges), **retained but detached** (the `users` row becomes a tombstone — `deleted-user-{id}@deleted.amphive.invalid`, name `Deleted user`, unusable password, `google_sub` NULL, `is_disabled`, `deleted_at` stamped, `token_version` bumped so every outstanding JWT dies). Any remaining charging credit is **forfeited** and written as an `account_closure` ledger line so the running balance still reconciles (credit is closed-loop and non-withdrawable — see the /charging-credit-terms page). **409** when it would leave the platform inconsistent: an ACTIVE charging session, or the caller is the last operator of a tenant that still has gateways registered. Audited as `account.close` with `actor_user_id=None`. Rate-limited per account (`DELETE_ACCOUNT_ACCOUNT_RATE_LIMIT`, default 5/3600). → `{status:"closed", forfeited_coins, detail}` |
...
| Method | Path | Auth | Body/Params | Behaviour |
|--------|------|------|-------------|-----------|
| POST | `/api/sessions/start` | JWT | `{plug_id, max_duration_seconds?, max_kwh?}` — `max_kwh` (0.1–100) / `max_duration_seconds` (1 s–24 h) are **OPT-IN user-set charging limits** (2026-07-12; **opt-in default 2026-08-02**): persisted onto the session (`ChargingSession.max_kwh`/`max_duration_seconds`, Alembic `0015_session_limits`) verbatim, including NULL, and — when set — **enforced by a backend auto-stop** ("auto-stopped: energy limit reached" / "auto-stopped: time limit reached", ~1 s after the limit via the telemetry path, env `AUTO_STOP_ON_LIMITS`; the firmware additionally enforces both locally as relay watchdogs, and the reaper carries a duration backstop). **Omitted fields mean "no limit" — the session charges until the driver stops it** (or a real safety net does: balance exhaustion, gateway-offline reaping, overcurrent, plug caps); NOT a hidden 14400 s/30 kWh default (that was the pre-2026-08-02 behavior). The gateway ON command always carries firmware-safe numeric watchdog values regardless — the driver's own limit, or an UNLIMITED sentinel when they set none (`services/mqtt_manager.py firmware_duration`/`firmware_max_kwh` — the firmware has no wire representation for "no limit": an omitted field falls back to ITS OWN old hard default, and 0 is an instant on-device cutoff, so neither is used) | Reject if the user already has `MAX_ACTIVE_SESSIONS_PER_USER` (env, default 2) ACTIVE sessions (409; counted under a user-row lock so concurrent starts can't exceed the cap) → access check → reject if OCCUPIED/offline/gateway-dead (409) → **reservation gate** (2026-07-12, under the plug row lock: lazy no-show expiry first, then if a BOOKED window covers now and belongs to someone else → 409 "Plug is reserved until <end>"; the holder's own start marks the reservation FULFILLED and links `session_id` — see [Plug reservations](#plug-reservations-routersreservationspy-2026-07-12)) → resolve the billing rate → size + require an authorization hold ≥ 50 (402; **2026-07-12** — `min(available_balance, max_kwh × rate)` when `max_kwh` is set, else just `available_balance` — `services/wallet.py available_balance`, replacing the old flat-balance floor — see MARKET_GAP_ANALYSIS.md §3; a smaller user `max_kwh` shrinks the hold too, and an unlimited session's hold is exactly its available balance, which is what balance-exhaustion auto-stop then fires against) → MQTT `ON` (500 on publish fail) → create session (snapshotting the hold onto `hold_coins` and the limits), mark plug OCCUPIED → `{status:"started", session_id, plug_id, plug_name, max_kwh, max_duration_seconds, message}` (the two limit fields echo the **effective** values — NULL when unlimited — added 2026-07-12) |
| POST | `/api/sessions/stop` | JWT | `{session_id}` | Owner+active check → MQTT `OFF` (best-effort) → finalize from telemetry → debit wallet → ledger `session_debit` → plug AVAILABLE → the receipt payload `{status:"completed", session_id, plug_id, plug_name, energy_kwh, peak_power_w, price_per_kwh, coins_spent, shortfall_coins, balance_before, balance_remaining, duration_sec, started_at, ended_at, max_kwh, max_duration_seconds, reason}` — `max_kwh`/`max_duration_seconds` (added 2026-07-12; NULL for an unlimited session — the default since 2026-08-02 — or a legacy pre-limit session) are the limits the session ran with, so the UI can say which one an auto-stop hit alongside `reason` |
...
| POST | `/api/cpo/gateways` | cpo/admin | `{gateway_id, name, vpn_ip}` | Register a gateway under the tenant directly (manual path — gateway_id is the device MAC, typed or copied off the setup portal). Still supported for lab/legacy units never minted as claim-code inventory. |
| POST | `/api/cpo/gateways/claim` | cpo/admin, rate-limited (`CPO_GATEWAY_CLAIM_ACCOUNT_RATE_LIMIT`, default 10/60 per account) | `{claim_code, name?}` | **Preflashed-unit onboarding (2026-08-02).** Binds an admin-minted unclaimed gateway (`POST /api/admin/gateways/inventory` below) to the caller's tenant — the primary "Add gateway" flow in the CPO portal. `claim_code` is normalized (whitespace/dashes stripped, uppercased) before lookup; optional `name` overrides the inventory placeholder name. Every failure — unknown code, already-claimed code, or malformed input — returns the SAME generic `404 {"detail":"Claim code not found or already used."}` (no enumeration oracle; mirrors `forgot_password`'s reasoning). → `{status:"claimed", gateway_id, name, vpn_ip}` |
| GET | `/api/cpo/firmware-releases` | cpo/admin | — | **(feat/ota-version-picker, 2026-08-02)** Active firmware releases for the OTA version picker, newest-version-first (semver-aware — `services/versioning.version_sort_key`, not a string sort: "2.10.0" ranks above "2.9.0"). Global catalog, not tenant-scoped. → bare list `[{id, version, url, notes, created_at}]` |
| POST | `/api/cpo/gateways/{id}/ota` | cpo/admin | `{release_id}` **or** `{firmware_url}` (https, ≤512 chars; exactly one required) | **(feat/ota-version-picker, 2026-08-02)** Trigger an OTA firmware update — the version-picker flow (`release_id`, resolved against `GET /api/cpo/firmware-releases`) replaced hand-pasting a URL; `firmware_url` remains as an admin-only escape hatch (403 for a non-admin caller). Requires the gateway `ONLINE` (409 if offline — gates on the status flag, **not** telemetry freshness, so a gateway with an unreachable plug can still be updated) and ≥1 plug (409 — the OTA command rides a per-plug command topic); `release_id` also 404s if the release doesn't exist or was deactivated. Publishes the `OTA` command (502 on publish fail); the gateway downloads into its passive slot and reboots (rollback-protected), refusing mid-session. → `{status:"ota_triggered", gateway_id, firmware_url, release_version, message}` |
...
| POST | `/api/admin/firmware-releases` | admin | `{version, url, notes?}` (`version` semver-shaped e.g. `2.4.0-direct`; `url` https, ≤512 chars) | **(feat/ota-version-picker, 2026-08-02)** Register a firmware release the CPO version picker can offer — does **not** upload/host the binary, `url` must already be a reachable published image (today: `gs://amphive-fw`, see [FIRMWARE.md](FIRMWARE.md) §7). 400 if `version` is already registered **and ACTIVE**; **since 2026-08-11 (PR #115)** re-registering a version that exists but is **DEACTIVATED** overwrites its `url`/`notes` and **reactivates** it (a deactivated release isn't permanently stranded — audited as `firmware_release.overwrite_reactivate` instead of `firmware_release.create`). → `{id, version, url, notes, is_active, created_at}` |
| POST | `/api/admin/firmware-releases/upload` | admin | query `notes?`; **raw body** (`application/octet-stream`, ≤4 MiB) = the **signed** `build/amphive-gateway.bin` | **(feat/admin-dashboard, 2026-08-04; rewired to GCS 2026-08-11, PR #115)** UI-based publish — the sibling of the register-a-URL endpoint above, but the backend **publishes** the binary for you. Validates the ESP image magic `0xE9` + the `esp_app_desc` magic `0xABCD5432` at offset 32, extracts `version` (offset 48) and `project_name` (offset 80 — must equal `amphive-gateway`) — version/filename come **from the image itself**, never from the client — then uploads the bytes to the public bucket **`gs://amphive-fw`** (`FIRMWARE_GCS_BUCKET`; keyless VM service-account ADC, `services/firmware_publish.py` — one-time IAM prerequisite in [deploy/docs/ota_image_publishing.md](../deploy/docs/ota_image_publishing.md)) and **auto-registers** the `FirmwareRelease` (`is_active`) with `url = https://storage.googleapis.com/amphive-fw/amphive-gateway-<version>.bin` — the same host every historically-successful OTA fetched from (the old self-hosted `/api/firmware/images/...` URL was unreliable for real devices and vanished on redeploy). **502** if the bucket publish fails (nothing registered). ACTIVE duplicate `version` → 400 (publishes nothing); a **DEACTIVATED** duplicate is **re-published + overwritten + reactivated** (audited as `firmware_release.reupload_reactivate` instead of `firmware_release.upload`). → the release row + `size_bytes` + `filename`. Publishing = build locally → drag the `.bin` into **Admin → Firmware Releases** → push from any gateway page picker; `deploy/scripts/publish_firmware.ps1` / manual `gcloud storage cp` remain as the fallback path (see [FIRMWARE.md](FIRMWARE.md) §7). |
...
| POST | `/api/admin/firmware-releases/{id}/reactivate` | admin | path `id:int` | **(2026-08-11, PR #115)** Re-enable a soft-deactivated release — the inverse of `deactivate` above (deactivation is no longer a one-way trap); it reappears in the CPO picker immediately. Idempotent (reactivating an already-active release just confirms the state). 404 if missing. Audited as `firmware_release.reactivate`. → `{status:"reactivated", id, version}` |
...
**Public firmware-image host (`routers/firmware_images.py`, feat/admin-dashboard, 2026-08-04 — legacy back-compat since 2026-08-11).**
`GET /api/firmware/images/{filename}` — **auth: none, unauthenticated by design** —
serves images stored under `FIRMWARE_IMAGE_DIR` by the pre-PR-#115 upload
endpoint. New uploads publish to `gs://amphive-fw` instead (see the upload row
above), so this endpoint remains only so any already-registered
`/api/firmware/images/...` release URL keeps resolving.
`filename` is constrained by a whitelist regex plus path containment (no
traversal). This is deliberate parity with the world-readable GCS
...
| `VERIFY_EMAIL_RATE_LIMIT` / `RESEND_VERIFICATION_RATE_LIMIT` / `RESEND_VERIFICATION_EMAIL_RATE_LIMIT` | `10/3600` / `5/3600` / `3/3600` | **(2026-08-11)** Email-verification limits — per-IP verify-email + resend, plus the per-submitted-email resend mailbomb cap (enumeration-safe, same format/mechanism as the forgot-password limits) |
...
| `APP_DB_PASSWORD` | `""` | **(2026-08-04, DB privilege separation)** When set, requests serve through a NON-superuser role (`APP_DB_USER`, default `amphive_app`, DML-only — cannot DROP/ALTER tables) instead of the `DATABASE_URL` owner; the app provisions the role at startup and verifies it before serving. Unset = prior behavior (serve as the owner role). See SECURITY.md §2 |
| `APP_DB_USER` | `amphive_app` | **(2026-08-04)** Name of the least-privilege runtime role (validated as a plain SQL identifier). Only used when `APP_DB_PASSWORD` is set |
| `PUBLIC_API_ORIGIN` | `""` | **(feat/admin-dashboard, 2026-08-04; unused since 2026-08-11, PR #115)** Was the base URL stamped into uploaded firmware-release image URLs (`/api/firmware/images/...`); uploads now register `gs://amphive-fw` bucket URLs, so the backend no longer reads this |
| `FIRMWARE_GCS_BUCKET` | `amphive-fw` | **(2026-08-11, PR #115)** Public-read GCS bucket `POST /api/admin/firmware-releases/upload` publishes `.bin` images to (keyless VM service-account ADC — `services/firmware_publish.py`; one-time IAM grant in [deploy/docs/ota_image_publishing.md](../deploy/docs/ota_image_publishing.md)) |
| `FIRMWARE_IMAGE_DIR` | `data/firmware-images` | **(feat/admin-dashboard, 2026-08-04)** Where the pre-PR-#115 upload endpoint stored `.bin` images (a docker volume in `docker-compose.relay.yml`); still served by the legacy back-compat `GET /api/firmware/images/{filename}` — new uploads go to `gs://amphive-fw` instead |
...
| `EMAIL_VERIFICATION_TTL_MIN` | `1440` | **(2026-08-11)** Minutes an emailed `verify-email` link stays valid (single use); 24 h default since a new user may not click immediately |
