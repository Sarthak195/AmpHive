...
- [~] **MQTT broker auth + TLS** — auth **staged 2026-07-07** (SEC §3):
      mosquitto `password_file` generated by `deploy.ps1` from `.env`
      (`MQTT_USERNAME/PASSWORD` backend + `MQTT_GW_USERNAME/PASSWORD` gateway
      fleet), authenticated compose healthcheck, backend env plumbed, firmware
      credentials field added (NVS `mqtt_user`/`mqtt_pwd` + optional portal
      inputs, anonymous fallback) and **seeded + flashed on the dev gateway**.
      **Remaining:** deploy, verify the gateway authenticates, then flip
      `allow_anonymous false` in `mosquitto.conf` + redeploy. TLS still future
      (overlay provides confidentiality today). (SEC §3)
