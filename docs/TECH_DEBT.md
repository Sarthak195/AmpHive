...
| 20 | ✅ ~~**ESP32 firmware actuates ONE plug regardless of the command's target**~~ **Done 2026-07-12 (code-complete + builds clean, on-device verify pending)** *(2026-07-06 audit)* | `firmware/main/main.c` + `firmware/main/tapo_protocol.c` + `firmware/main/session_nvs.c` | Was single-instance (one global KLAP session + energy integrator; ON/OFF always drove `target_plug_ip`; telemetry under the last-commanded `active_plug_id`). Fixed: `main.c` holds a `plugs_mutex`-guarded per-plug slot table; `tapo_protocol.c` exposes a per-plug `tapo_plug_t` context (own KLAP session + NVS energy meter `wh_<plug_id>`); `session_nvs` persists all sessions in one atomic blob (each carrying `plug_id` + `local_ip`). The backend ships `plugs.local_ip` on every ON/OFF so the gateway drives the right plug + learns unseen ones, with no on-device roster — per-gateway broker ACLs and the `plug.gateway_id` billing check unchanged (SEC §8.5). Verified by construction + ESP-IDF v5.3.3 build; two-real-plug on-device test needs a second unit. | — | ~~P1~~ Done |
