...
| 33 | ✅ **Split `cpo.py` god router and `MQTTManager` god object** — **Resolved 2026-07-21 (PR #78)**: `routers/cpo/` package (12 modules, 106-route parity) + `services/mqtt/` facade with 7 mixins (telemetry/status/alarms/logs/config/…) | `backend/routers/cpo.py` (~1,200 loc), `backend/services/mqtt_manager.py` | Both files aggregate multiple concerns — `cpo.py` covers gateway CRUD, audit, analytics, events, maintenance, tariffs, and payouts in one endpoint bucket; `mqtt_manager.py` handles subscription, telemetry dispatch, session lifecycle, and reprice logic (see IMPLEMENTATION_STATUS §3 routing notes). | Each remains functional but harder to test, review, and onboard on. Splitting into single-responsibility routers (`cpo_gateways`, `cpo_analytics`, `cpo_tariffs`, etc.) and MQTT topic handlers (telemetry, status, alarms, logs, config) would clarify ownership and reduce cognitive load. | ~2–3 h per file (routing + test rewire) | **P2** |
